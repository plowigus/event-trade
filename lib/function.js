...
export async function fetchOfferPosts() {
  try {
    ...
    const offers = data.posts.edges.map((edge) => {
      ...
      // Wyciągnij tagi z WordPress
      const tags =
        edge.node.tags?.nodes?.map((tag) => ({
          id: tag.id,
          name: tag.name,
          slug: tag.slug,
        })) || [];

      // Wyciągnij kategorie z WordPress
      const categories =
      ...
      // Znajdź subkategorie (kategorie które mają parent "oferta")
      const subcategories = categories.filter(
        (cat) =>
          cat.parent &&
          (cat.parent.slug === "oferta" ||
            cat.parent.name.toLowerCase() === "oferta")
      );
...
// Funkcja do znajdowania realizacji powiązanych z ofertą na podstawie tagów z WordPress
export async function fetchRelatedRealizations(offerSlug, limit = 6) {
  try {
    // Pobierz konkretną ofertę żeby wyciągnąć jej tagi
    const offer = await fetchOfferBySlug(offerSlug);

    if (!offer || !offer.tags || offer.tags.length === 0) {
      console.log(`No tags found for offer slug: ${offerSlug}`);
      return {
        relatedRealizations: [],
        totalRelated: 0,
        searchedTags: [],
    ...
    const allRealizationsData = await fetchRealizationData();

    // Wyciągnij tagi z oferty
    const offerTags = offer.tags.map((tag) => tag.name.toLowerCase());
    const offerTagSlugs = offer.tags.map((tag) => tag.slug.toLowerCase());

    console.log(`Offer "${offerSlug}" has tags:`, offerTags);

    // Znajdź realizacje które mają wspólne tagi z ofertą
    const relatedRealizations = allRealizationsData.realizations.filter(
      (realization) => {
        // Sprawdź czy realizacja ma przynajmniej jeden wspólny tag z ofertą
        return realization.tags.some((realizationTag) => {
          const realizationTagName = realizationTag.name.toLowerCase();
          const realizationTagSlug = realizationTag.slug.toLowerCase();

          // Sprawdź dokładne dopasowanie nazwy lub slug
          return (
            offerTags.includes(realizationTagName) ||
            offerTagSlugs.includes(realizationTagSlug) ||
            // Sprawdź też czy tag realizacji zawiera się w tagu oferty lub na odwrót
            offerTags.some(
              (offerTag) =>
                realizationTagName.includes(offerTag) ||
                offerTag.includes(realizationTagName)
            )
          );
        });
      }
    ...
    // Posortuj po liczbie wspólnych tagów (malejąco) i ogranicz liczbę
    const limitedRealizations = relatedRealizations
      .map((realization) => {
        // Znajdź wszystkie wspólne tagi
        const matchedTags = realization.tags.filter((realizationTag) => {
          const realizationTagName = realizationTag.name.toLowerCase();
          const realizationTagSlug = realizationTag.slug.toLowerCase();
        ...
        return {
          ...
          heroImage: realization.heroImage,
          heroText: realization.heroText,
          company: realization.company,
      ...
      .sort((a, b) => b.matchedTagsCount - a.matchedTagsCount) // Sortuj po liczbie wspólnych tagów
      .slice(0, limit);

    console.log(
      `Found ${limitedRealizations.length} related realizations for ${offerSlug}`
    );
    console.log(`Offer tags:`, offerTags);

    return {
      relatedRealizations: limitedRealizations,
      totalRelated: limitedRealizations.length,
      searchedTags: offerTags,
      offerTags: offer.tags,
    };
  } catch (error) {
    console.error(
      `Error fetching related realizations for ${offerSlug}:`,
      error
    );
    return {
      relatedRealizations: [],
