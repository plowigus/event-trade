  ...
  return (
    <>
      <div className="about-section-bg w-full h-auto relative overflow-hidden">
        ...
        <div
          ref={glowRef}
          className="absolute top-0 right-0 w-40 h-full bg-[#c0368b25] opacity-30 z-[35] blur-xl"
        ></div>

        {/* Główny różowy pasek wertykalny z GSAP animacją */}
        <div
          ref={barRef}
          className="absolute top-0 right-0 w-32 h-full bg-[#c0368ba9] opacity-80 z-[36] shadow-lg"
        ></div>
        ...
        {Array.from({ length: 30 }).map((_, index) => (
          <div
            ...
            className="absolute w-4 h-4 bg-[#ffffff] rounded-full opacity-60 z-[34]"
            style={{
        ...
        <Image
          ...
          className="absolute z-[35] top-0 right-0 object-contain self-end"
        />
        {/* Header grid system */}
        <div className="grid grid-cols-12  pt-16 z-[31] relative">
          <div className="col-start-1 col-span-10 bg-[#C0368B] text-white">
            {/* Grid wewnętrzny dla pozycjonowania tekstu */}

            <div className="grid grid-cols-9 py-4 px-12">
              <h2 className="col-start-3 col-span-7 text-2xl lg:text-3xl uppercase tracking-widest font-museo">
                {data.title}
              </h2>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-10 gap-8 py-10 z-[31] relative">
          <div className="col-start-3 col-span-4">
            <p className="font-museo text-3xl uppercase mb-8 text-white">
              {data.subtitle}
            </p>
            <p className="font-museo text-xl uppercase mb-8 text-white">
              {data.text}
            </p>
            <p className="font-chillax text-md font-medium text-white">
              {data.subtext}
            </p>
          </div>
        </div>
