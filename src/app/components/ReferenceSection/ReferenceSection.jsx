...
export default function ReferenceSection() {
  ...
  useEffect(() => {
    if (references.length > 0) {
      // Ustawienie początkowej opacity bez animacji
      if (referenceContentRef.current) {
        gsap.set(referenceContentRef.current, {
          opacity: 1,
          y: 0,
        });
      }
      ...
      if (parallaxBoxRef.current && logoGridRef.current) {
        setTimeout(() => {
          ...
          // Calculate exact distance with better precision
          const boxRect = parallaxBoxRef.current.getBoundingClientRect();
          const logoGridRect = logoGridRef.current.getBoundingClientRect();
          const boxTop = parallaxBoxRef.current.offsetTop;
          const logoGridTop = logoGridRef.current.offsetTop;

          // Dokładna odległość którą box musi przejechać + dwa rzędy niżej
          // Każdy rząd ma około 92px (20px wysokość + 48px gap)
          const twoRowsDown = 165; // 2 * 92px
          const exactDistance = logoGridTop - boxTop + twoRowsDown;

          // Oblicz długość scroll dla animacji
          const scrollDistance = window.innerHeight + exactDistance;
          ...
          gsap.to(parallaxBoxRef.current, {
            ...
            scrollTrigger: {
              ...
              start: "top top", // Zaczyna gdy góra boxa dotknie góry ekranu
              end: `+=${exactDistance}`, // Kończy po przejechaniu dokładnej odległości
              scrub: 2.5, // Płynne przesuwanie
              markers: true,
  ...
  useEffect(() => {
    if (
      ...
      // Animacja pojawiania się tekstu tylko przy zmianie referencji
  ...
  return (
    <div className="bg-black pt-8 min-h-screen relative" ref={sectionRef}>
      <div className="container mx-auto px-8">
        {/* Sekcja referencji - dynamiczny parallax */}
        <div
          className="mb-16 h-96 py-20 flex items-center justify-center bg-black z-10 relative"
          ref={parallaxBoxRef}
        >
          {displayedReference && (
